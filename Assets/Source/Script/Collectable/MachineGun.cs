...
public class MachineGun : MonoBehaviour
{
    ...
    public GunStat machineGunStat;
    public GameObject machineBulletPrefabs;
    ...
    private void Awake()
    {
        ...
        m_countBullet = machineGunStat.countBullet;
    }
    ...
    private void GunRotation()
    {
        ...
        {
            transform.position = new Vector3(m_player.transform.position.x + 1f, m_player.transform.position.y - 6.5f, m_player.transform.position.z + 0f);
        }
    ...
    private void FireBulletGunChecking()
    {
        ...
        if (m_isAttacked)
        {
            m_player.ReduceTimeAction(ref m_isAttacked, ref m_currTimeAttack, machineGunStat.rateTimeFireBullet);
        }
    }
    ...
    private void FireBullet()
    {
        ...
        if (machineBulletPrefabs != null)
        {
            GameObject bulletClone = GameObject.Instantiate(machineBulletPrefabs, bulletSpawnPos.position, Quaternion.identity);
            MachineBullet machinBullet = bulletClone.gameObject.GetComponent<MachineBullet>();
            machinBullet.GunObj = gameObject;

            m_countBullet -= 1;

            //Rigidbody2D rb = machinBullet.GetComponent<Rigidbody2D>();
